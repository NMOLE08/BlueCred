  ...
  const listTokens = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userBalance < ethers.parseEther(newListing.tokenAmount)) {
        throw new Error(`Insufficient tokens. You have ${ethers.formatEther(userBalance)} NCT but trying to list ${newListing.tokenAmount} NCT`);
      }
      
      // Check current allowance first
      const tokenAmount = ethers.parseEther(newListing.tokenAmount);
      const currentAllowance = await carbonToken.allowance(user.address, await marketplace.getAddress());
